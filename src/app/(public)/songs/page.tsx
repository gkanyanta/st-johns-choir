    ...
    <div>
      ...
      <div className="bg-slate-900 py-20 px-4 sm:px-6 text-center relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-white">Songs & Media</h1>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-16">
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <h3 className="text-lg font-semibold text-slate-400">No songs yet</h3>
            <p className="text-slate-400 mt-1">Check back later for our music and videos</p>
          </div>
        ) : (
          <div className="space-y-12">
            ...
            {activeVideo && (
              <div className="max-w-3xl mx-auto">
                <div className="relative w-full rounded-xl overflow-hidden shadow-xl ring-1 ring-stone-200/60" style={{ paddingBottom: "56.25%" }}>
                  <iframe
                    ...
                    {videoId ? (
                      ...
                      >
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                          <div className="bg-amber-500 rounded-full p-3 group-hover:scale-110 transition-transform shadow-lg shadow-amber-500/30">
                            <Play className="h-6 w-6 text-white fill-white" />
                          </div>
                        </div>
                      </button>
                    ) : (
                      <div className="w-full aspect-video bg-gradient-to-br from-stone-100 to-stone-50 flex items-center justify-center">
                        <Music className="h-12 w-12 text-stone-300" />
                      </div>
                    )}
                    <div className="p-5">
                      <h3 className="font-semibold text-slate-900">{song.title}</h3>
                      {song.artist && <p className="text-sm text-slate-400 mt-0.5">{song.artist}</p>}
                      <div className="flex items-center gap-2 mt-3">
                        ...
                        {videoId && (
                          <a
                            ...
                            className="text-xs text-slate-400 hover:text-red-600 flex items-center gap-1 ml-auto transition-colors"
                          >
                      ...
                      {song.description && (
                        <p className="text-xs text-slate-400 mt-2 line-clamp-2">{song.description}</p>
                      )}
                    </div>
                  </div>
                );
